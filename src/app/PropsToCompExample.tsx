...
function getImageUrl(person: Person): string { return './' + person.imgId + '.ico' }
interface CardProps { children: React.ReactNode }
function Card({ children }: CardProps) { return <div>{children}</div> }
interface AvatarProps { person: Person, size: number }
function Avatar({ person, size }: AvatarProps): JSX.Element { return <img src={getImageUrl(person)} alt={person.name} width={size} />; }
